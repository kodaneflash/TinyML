...
tm.save_model(model, "news-sentiment-predictor")

# Step 5: load a saved model and use it
loaded_model = tm.load_model("news-sentiment-predictor.tar.gz")
```

## 2. Features

`TinyML` combines graph search, LLM code/data generation and code execution to produce a machine learning model
...
The model can now be used to make predictions, and can be saved or loaded using `tm.save_model()` or `tm.load_model()`.
...
model = tm.Model(intent="Predict sentiment on a news article such that [...]")
